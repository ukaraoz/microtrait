...
rds_dir = file.path(base, "organisms_habitatfiltered_matchedbyanalysis_wIMGTaxonID_taxonids")
rds_files = list.files(rds_dir, full.names = T, recursive = F, pattern = ".microtrait.rds$")
ncores = 20
...
gp_results = readRDS(file.path(base, paste0(dataset, ".gp.rds")))
genomeset_results_wmetadata = add.metadata(genomeset_results, gp_results, genome_metadata)
